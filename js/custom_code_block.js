...
KEEP.initCodeBlock = () => {
	if (KEEP.theme_config?.code_block?.tools?.enable === true) {
		KEEP.utils.initCodeBlockTools = () => {
			const { style: codeBlockStyle } =
				KEEP.theme_config?.code_block || {};
			const { style: codeBlockToolsStyle } =
				KEEP.theme_config?.code_block?.tools || {};

			const isMac =
				(codeBlockStyle || codeBlockToolsStyle || "default") === "mac";
			const foldedIconClassName = isMac
				? "fas fa-chevron-left"
			...
			document.querySelectorAll("figure.highlight").forEach(element => {
				// code language
				let codeLang = element.classList.length
					? element.classList[1].toUpperCase()
					: "";
          ...
          <span class="tool copy tooltip" data-tooltip-content="${copyLang}" data-tooltip-offset-y="-2px"><i class="fas fa-copy"></i></span>
				...
				copyDom.addEventListener("click", event => {
					...
					const copyTooltipDom = codeToolsBox.querySelector(
					...
					if (result) {
						...
						copyTooltipDom &&
							(copyTooltipDom.innerHTML = copiedLang);
					} else {
				...
				targetFoldDom.addEventListener("click", event => {
					...
					const foldTooltipDom = codeToolsBox.querySelector(
					...
					if (isFold) {
						...
						foldTooltipDom &&
							(foldTooltipDom.innerHTML = foldedLang);
					} else {
	...
	if (
		postContentDom &&
		!postContentDom.classList.contains("code-block-unshrink")
	) {
		KEEP.utils.shrinkCodeBlock = () => {
			const shrinkHandle = codeBox => {
				...
				if (codeBoxHeight - limitHeight > 1000) {
					...
					shrinkLineDom.setAttribute(
						"class",
						"shrink-line flex-center"
					);
